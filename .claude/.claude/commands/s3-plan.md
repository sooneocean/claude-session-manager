...
architect 完成 implementation plan 後、S3 Gate 摘要通知前，寫入 checkpoint node：

1. 寫入 `{spec_folder}/nodes/node-{NNN}-s3-plan.md`：
   - Conclusion: "Plan: {N} waves, {M} tasks. Complexity: {low/medium/high}."
   - Artifacts: s3_implementation_plan.md path
   - Next Input: Wave-by-wave task summary (ID + description + agent). If >2KB: "see s3_implementation_plan.md"
   - Gate: Result=auto_continue, Next Stage=S4, Next Action="Start Wave 1 implementation."
2. Update CURRENT
