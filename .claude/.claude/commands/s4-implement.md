---
description: "S4 實作 — 依執行計畫進行程式碼實作"
...
全部 Wave 完成後（不是每個任務），寫入 checkpoint node：

**正常完成**：
- node-{NNN}-s4-impl.md
- Conclusion: "All {N} tasks completed. {M} files changed. Build: {pass/fail}."
- Artifacts: list of changed files
- Next Input: Changed files list (scoped for S5 review). "Changes ready for code review."
- Gate: Result=completed, Next Stage=S5

**S4↔S5 Repair（S5 P1 回來時）**：
- node-{NNN}-s4-repair-{round}.md
- Conclusion: "Fixed {N} P1 issues (repair round {M}/3)."
- Next Input: Repaired files for re-review
- Gate: Result=completed, Next Stage=S5

**S4↔S6 Defect Fix（S6 fail 回來時）**：
- node-{NNN}-s4-defect-fix-{round}.md
- Conclusion: "Fixed {N} defects (defect fix round {M}/3)."
- Next Input: Repaired files for re-test
- Gate: Result=completed, Next Stage=S6

Write order: node → CURRENT → sdd_context. Do NOT write a node per-task or per-wave.
