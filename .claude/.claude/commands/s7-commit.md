---
description: "S7 提交 — 整理變更並 git commit"
...
git commit 完成後，寫入 checkpoint node：

1. 寫入 `{spec_folder}/nodes/node-{NNN}-s7-commit.md`：
   - Conclusion: "Committed: {hash} on branch {branch}. {N} files, +{added}/-{deleted} lines."
   - Artifacts: commit hash

   **Non-ATW Next Input**: "SOP complete. No further action."

   **ATW Next Input**（`dev/atw-session.json` 存在且 status=in_progress）：
   ```
   ### Feature Summary (for ATW handoff)
   - **Feature**: {feature name}
   - **What was built**: {1-2 sentence}
   - **Key artifacts**: {spec + key source files}
   - **Key decisions**: {top 2-3}
   - **Commit**: {hash} on {branch}

   ### ATW Next
   - **Next Feature**: #{id} — {description}
   - **Action**: Compact context, then start S0 for next feature.
   ```

   - Gate: Result=done, Next Stage=DONE

2. Update CURRENT
3. 繼續 SDD Context 持久化
