---
description: "S2 Spec 審查 — 對抗式審查 dev_spec"
...
R1~R3 完成後（或 Quick 跳過時），寫入 checkpoint node：

**Full Spec（審查完成）**：
- node-{NNN}-s2-review.md / node-{NNN}-s2-review-conditional.md / node-{NNN}-s2-review-fail.md / node-{NNN}-s2-review-disputed.md
- Conclusion: "Review verdict: {pass/conditional_pass/fail/disputed}. {N} findings (P0={n}/P1={n}/P2={n})."
- Next Input: pass→"Spec approved, proceed to planning." / conditional→P2 items list / fail→critical findings / disputed→both positions
- Gate: Result={passed/conditional/fail/disputed}

**Quick（跳過 S2）**：
- node-{NNN}-s2-skip.md, stage_status=skipped
- Conclusion: "Quick mode: S2 skipped."
- Next Input: "Proceed directly to S3 planning."
- Gate: Result=skipped
