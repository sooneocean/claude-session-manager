...
測試完成後，依結果寫入 checkpoint node：

**Pass（含 audit-converge 通過）**：
- node-{NNN}-s6-test-pass.md
- Conclusion: "All tests passed. {N} automated, {M} e2e. Defects: 0. Audit-converge: converged."
- Next Input: Changes summary for commit. Feature name + type for commit message. "Proceed to S7 commit."
- Gate: Result=passed, Next Stage=S7

**Fail**：
- node-{NNN}-s6-test-fail.md
- Conclusion: "Tests failed. {N} defects found. Repair loop {M}/3."
- Next Input: Defect list (TC ID + description + error). "Return to S4 for defect repair."
- Gate: Result=fail, Next Stage=S4
